...
//            exit();
    ...
    function get_top_rated_knows($data)
    {
        ...
        $sql_query = " 
            select a.*, b.* from user_people as a 
            inner join  (select user_id, sum(ranking) as rate from user_rating group by user_id) as b on a.id = b.user_id 
            where b.rate = '$ranking' and a.client_email like '%$search_email%' and 
                (a.client_name like '%$name_profession%' or a.client_profession like '%$name_profession%')
            order by a.client_name asc limit $offset, $pagesize";
