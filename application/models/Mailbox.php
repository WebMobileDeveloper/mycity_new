...
class Mailbox extends CI_Model
{
	...
	function get_inbox($data)
	{
		...
		$sender ='';
		$sql_query = "select a.subject, a.emailbody  , a.senton  , a.email_type  ,  a.emailstatus  , " . 
		" a.id , a.sender , a.receipent , b.id as partnerid, b.username  , b.user_pkg  , b.user_phone  , b.user_email, 0 connect_status  from  mc_mailbox as a " .
		" inner join mc_user as b on a.sender=b.user_email where receipent='$receipent' and email_type='$mailtype'  and b.user_status='1' order by a.senton desc limit $offset,10";
		...
		if($result->num_rows() > 0 )
		{
			...
			$jsonresult = array('error' => '0', 'errmsg' => 'Mails are retrieved!',  'num_rows' => $pages, 
			'result' =>  $result ); 
		}
		else
		{
			$jsonresult = array('error' =>  '10' ,  'errmsg' =>   'No email found!', 'result' =>  null );
		} 
		return   ( $jsonresult  ); 
	}
	...
	function get_outbox($data)
	{
		...
		$sql_query = "select a.subject, a.emailbody  , a.senton  , a.email_type  ,  a.emailstatus  , " . 
		" a.id , a.sender ,  a.receipent , b.id as partnerid, b.username  , b.user_pkg  , b.user_phone  , b.user_email , 0 connect_status from  mc_mailbox as a " .
		" inner join mc_user as b on a.receipent=b.user_email where sender='$receipent' and email_type='$mailtype' and b.user_status='1' order by a.senton desc limit $offset,10";
		
		$sql_query_count = "select count(*) as reccnt  from  mc_mailbox as a inner join mc_user as b on a.sender=b.user_email where  sender='$receipent' and email_type='$mailtype' and b.user_status='1' ";
		$result = $this->db->query($sql_query);   
		if($result->num_rows() > 0 )
		{
			$result_count = $this->db->query($sql_query_count);  
