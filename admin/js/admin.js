...
function pareparetimelinecanvas(page1, page2, page3, client="") {
    ...
    $('#act_clients').empty();
    $('#inact_clients').empty();
    var BASE_URL = (window.location.hostname == 'localhost') ? 'localhost' : 'https://mycity.com';
    $.ajax({
        type: 'post',
        ...
        success: function (data) {
            ...
            $.each(data.results, function (index, item) {
                ...
                user_picture = !( item.h ) ? "images/no-photo.png" : BASE_URL + "/assets/uploads/profiles/" + item.h;
                html += "<tr id='row" + index + "'>" +
