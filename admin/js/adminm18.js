...
$(document).on('click', '.reversetrackpartner', function () {
    ...
    reloadreversetrackknow(searchkey, vocations, location, filtertags, filterlifestyle, 1)
});
...
$(document).on('click', '.reversetrackpager li a', function () {
    ...
    var filtertags = $('#reversetracktags').val() + '';
    var filterlifestyle = $('#reversetracklifestyle').val() + '';
    reloadreversetrackknow(searchkey, vocations, location, filtertags, filterlifestyle, goto)
});


function reloadreversetrackknow(searchkey, vocations, location, tags, lifestyle, goto) {
    $.ajax({
        type: 'post',
            ...
            if (data.error == 1) {
                ...
                $.each(data.results, function (index, item) {
                    ...
                    html += "<tr id='$rand-$id'>" +
                        ...
                        "<strong>City:</strong> " + (   item.client_location != null ? item.client_location : "not provided" ) + " <strong>ZIP:</strong> " + item.client_zip + "<br/>" +
...
function pareparetimelinecanvas(page1, page2, page3, client='sd') {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data, function (outindex, outitem) {
                ...
                else if (outindex == 'email') {
                    html += '<td>Email</td>';
                    html += '<td>' + outitem[0].item +
                        '<input type="hidden" value="' + outitem[0].item + '" id="bcemail">' +
                        '</td>';
                }
                else if (outindex == 'organization') {
