...
$param = array('id' => '0');
$allcities = json_decode(   curlexecute($param, $siteurl . 'api/api.php/cities/'), true);  
$getallGroups = json_decode(   curlexecute($param, $siteurl . 'api/api.php/groups/'), true);   
...
if (isset($_POST['updProf']) && isset($_POST['reg_email'])) {
	...
	if($insQ)
	{
		...
		$qryStmnt = "INSERT INTO user_details (user_id, street,  city, zip, country, lcid) VALUES ('$insID', '$reg_street', '$reg_city','$reg_zip', '$reg_country', $lcid)";
        $link->query($qryStmnt);
		...
		<title>Thank for registering on mycity.com.</title>
		<style type='text/css'>
...
<table border='0' cellpadding='0' cellspacing='0' width='100%'>
    <tr>
            ...
            <table border='0' cellpadding='0' cellspacing='0' align='center' width='640' bgcolor='#FFFFFF' style='color: #333333'>
                ...
                <tr>
                    <td style='padding: 10px 10px 30px 10px;'>
                        <div  >Hi <span style='font-weight: bold;'>".$reg_first_name . " " . $reg_last_name.",</span> 
                        <br />
                        <p>Thank for registering on mycity.com.</p> 
						<p style='line-height: 1.7'>Mycity is a referral system to help you grow your clients and referral partners.</p>
						<p style='line-height: 1.7'>Many call Mycity the match.com of the business world.</p>
						<p style='line-height: 1.7'>Mycity has many great tools to increase communications. Many social networking sites like LinkedIn people receive extremely low response rates.</p>
						<p style='line-height: 1.7'>Having a 30 year history in sales I follow these rules, which I call \"The Power of 3\". You can quickly take people you meet either online or face to face meeting with prospect to relationship as a client or referral partner.</p>

						<ol style='line-height: 1.7'>
						<li>First meeting, help, don't sell!</li>
						<li>Send them an introduction to someone they wish to meet.</li>
						<li>Follow up. See how the introduction went or even Invite them to a mixer or networking event.</li></ol>
						<p style='line-height: 1.7'>What you are showing them is you Care about them!!</p>
						<p style='line-height: 1.7'>If you have any questions, please contact me.</p>
						<p style='line-height: 1.7'>Bob Friedenthal<br />
       ...
       sendemail(  $reg_email ,  'Thank for registering on mycity.com.' , $msg, $msg );
...
//address update 
if (isset($_POST['reg_update_address'])) {
    $data = $_POST;
    $Msg = '';
	...
	{  
        $_SESSION['user_group'] = $allGrps;
		$qryStmnt = "insert into user_details ( country, zip, city, street, user_id) values  ('$reg_country', '$reg_zip', '$reg_city',  '$reg_street', '$insID' )";
        $link->query($qryStmnt);
...
if (isset($_POST['regdet_update'])) {
	...
	{ 
        ...
        $_SESSION['user_group'] = $allGrps;
         $qryStmnt = "update user_details set 
		groups='$groups_result', target_clients='$target_clients', target_referral_partners='$target_referral_partners', 
		vocations='$vocation_result' where user_id ='$insID' "; 
        $link->query($qryStmnt);
    ...
    if($q->num_rows > 0)
	{
        ...
        $html .= "</select>
				...
				<div class='col-xs-12 col-sm-12 padd-5'>
                    ...
                    <div class='col-sm-12 col-xs-12'> 
					<select name='e_lifestyle' multiple class='form-control chosen-select ed_client_lifestyle' >
					<option value='0'>Select Lifestyle</option>";
					
					$currentlifestyles = explode(",", $client_lifestyle);  
					$lifestyles  = getLifestyles($link);
					foreach ($lifestyles as $lifestyle)
					{
						$lsselected = "";
						
						for($v=0; $v < sizeof($currentlifestyles); $v++)
						{
							if(strcmp( trim($currentlifestyles[$v]),   trim($lifestyle['name']) ) == 0 )
								$lsselected ='selected';
						}
						
						$html .= "<option " .$lsselected . " value='" .  $lifestyle['name']. "'>" .  $lifestyle['name'] . "</option>"; 
			...
			$html .="</select>
                ...
                <div class='col-xs-12 col-sm-12 padd-5'>
                    ...
                    <div class='col-sm-12 col-xs-12 chosen-select-lg'>
                        
						<select data-placeholder='Specify Cities' multiple name='ed_client_location'  id='' class='form-control chosen-select  ed_client_location client_location' > 
                            <option value=''>--Select Location---</option>";
                        
						$locations = explode(",", $location);  
						foreach ($getallGroups as $city)	 
						{
							$isselected ='';
							for($v=0; $v < sizeof($locations); $v++)
							{
								if(strcmp( trim($locations[$v]),  trim($city['grp_name']) ) == 0 )
									$isselected='selected';
							}  
							$html .= "<option " .$isselected . " value='" . $city['grp_name'] . "'>" . $city['grp_name'] . "</option>"; 
						}
						
				$html .="</select>
					<strong class='pull-right'>( In case your city is not listed, please make a request to list it <a data-toggle='tab' data-dismiss='modal' aria-label='Close' href='#menu65'>here</a>.)</strong>
                    </div>
					<div class='col-sm-12 col-xs-12'>
        ...
        foreach ($ques_data as $item)
		{
            ...
            elseif($q_type == 'text')
            {
				...
				$source_arr = array_filter(explode(",", $profession)) ;
				$where_com_voc = " where (FIND_IN_SET('".implode("', member_voc ) OR FIND_IN_SET('", $source_arr)."', member_voc ))";
				$targetvocs_arr = array(); 
				$sql_query =  " select * from mc_common_vocation "  . $where_com_voc;
				$rst = $link->query($sql_query); 
				if($rst->num_rows  > 0 )
				{
					while($row = $rst->fetch_array()) 
					{
						$targetvocs_arr[] = $row['know_common_voc'];
					}
				}
				//$interestedvocations = explode(",", $que_answer); 
				$interestedvocations = array_filter ( explode(',',  implode(',', $targetvocs_arr) . "," .  $que_answer ) ) ;
				
				
                $html .= "<div class='col-sm-12 col-xs-12 padd-5'>
                <select id='answer$q_id' data-ques='$q_id' name='$name'  data-placeholder='Choose vocations ...' class='form-control user_ques_text_ed user_target_voc_ed' multiple  >";
        ...
        echo $html .= "</div><div class='modal-footer'><div class='col-sm-12'><br><input type='button' value='UPDATE' id='$id' class='btn btn-primary btnblock pull-right updClientUser'></div><div class='clearfix'></div></div>";
    }
}
...
//reading mail 
if(isset($_POST['readinmail']))
{
    $mailid =$_POST['mailid']; 
    $query = "select a.*, b.id as uid from mc_mailbox as a inner join mc_user as b on a.sender=b.user_email where a.id='$mailid'  ";
    $mails = $link->query($query);
    if ($mails->num_rows > 0)
    {  
        $html ='<div class="contentout">';  
        $row = $mails->fetch_array(); 
        $html .='<div class="contenthead"><h3>' .   $row['subject'] . '</h3></div>';  
        $html .='<div class="contentin">' .   $row['emailbody'] . '</div>'; 
		$html .='<hr/>';
		$html .='<div class="contentfoot"><button class="btn btn-primary btncomposedirectmail" data-id="' .   $row['uid']  .  '">Reply Message</button></div>'; 
        $html .='</div>'; 
    } 
    echo $html; 
}
...
if(isset($_POST['changeAccSett'], $_SESSION['user_email']))
{
    ...
    if ($q->num_rows > 0)
	{
		...
		$busi_location_street = $row['busi_location_street'];
		$busi_location = $row['busi_location'];
		$busi_type =  $row['busi_type'] ; 
		$busi_hours = $row['busi_hours'] ;
		$busi_website = $row['busi_website'] ;
...
if(isset($_POST['upd_username'], $_SESSION['user_email']))
{
	...
	$linkedin_profile = $link->real_escape_string($_POST['linkedin_profile']);


    $is_business = $_POST['is_business'];
    $busi_name = $link->real_escape_string($_POST['busi_name']);
	$busi_location_street = $link->real_escape_string($_POST['busi_location_street']);
    $busi_location = $link->real_escape_string($_POST['busi_location']);
    $busi_type = $link->real_escape_string($_POST['busi_type']);
    $busi_hours= $link->real_escape_string($_POST['busi_hours']);
    $busi_website = $link->real_escape_string($_POST['busi_website']);
    $usertags = $link->real_escape_string($_POST['usertags']);  
	...
	//create shortcode
	$meminfo = $link->query("select username from mc_user where id='$data_id'");
	$muname  = $meminfo->fetch_array()['username'];
	
	if($muname !='')
	{
		$username_parts  =  explode(' ', $muname);
    ...
    if($is_business==0)
        $link->query("UPDATE mc_user SET user_email = '$upd_email', user_phone = '$upd_phone', username = '$upd_username',  tags='$usertags', profileisvisible='$profile_visible', user_shortcode='$short_code_name' WHERE id = '$data_id' ");
    else 
        $link->query("UPDATE mc_user SET user_email = '$upd_email', user_phone = '$upd_phone', user_type='1', profileisvisible='$profile_visible', user_shortcode='$short_code_name',  busi_name='$busi_name', busi_location='$busi_location', busi_location_street='$busi_location_street',  busi_type='$busi_type', busi_hours='$busi_hours', busi_website='$busi_website', tags='$usertags' WHERE id = '$data_id' ");
    ...
    if( $reccnt->fetch_array()['reccnt'] == 0)
    {
        ...
        (user_id,street, city, zip,country, groups, target_clients,  target_referral_partners, 
        vocations,  about_your_self ,  upd_reminder_email, upd_public_private , linkedin_profile) values 
        ('$data_id', '$upd_street', '$upd_city', '$upd_zip', '$upd_country', '$groups', '$target', '$referral', 
        '$voc' , '$about_your_self', '$upd_reminder_email', '$upd_public_private', '$linkedin_profile' )"); 
    }
    else 
    {
        $link->query("UPDATE user_details SET street='$upd_street', city = '$upd_city', zip = '$upd_zip', 
        country = '$upd_country', groups = '$groups', target_clients = '$target', target_referral_partners = '$referral', vocations = '$voc' ,about_your_self = '$about_your_self',upd_reminder_email='$upd_reminder_email',upd_public_private='$upd_public_private' , linkedin_profile='$linkedin_profile' WHERE user_id = '$data_id' ");
	...
	if ($lifestyle != ''  ) {
		...
		if($voc != ''  )
		{
			...
			for($i=0; $i < sizeof($arrvoc); $i++)
			{
				...
				$q = $link->query("select * from  mc_fuzzy_map  where  input_text = '" .  $arrvoc[$i] . "'"); 
				if($q->num_rows > 0)
				{
					$j=0;
					$where .= " OR "; 
...
if(isset($_POST['readmailogs']))
{
	...
	$allrecords =  $link->query("SELECT r.id, r.partnerid, r.knowtorefer, r.knowreferedto, r.entrydate, r.ranking, r.emaillog , r.emailstatus , r.sourcezip, r.targetzip , r.distance , r.distancecalculated , u.user_email, u.username, u.user_phone, u.image, u.user_status, 0 as marked 
    ...
    (r.distance >=  0 and r.distance < '30') and distancecalculated='1' " . $startwhere . " ORDER BY up.showfirst desc, r.id DESC ");
	
	
	$allrecordscount =  $link->query("SELECT count(*) as allcnt FROM referralsuggestions as r  inner join mc_user as u 
    ON r.partnerid= u.id inner join user_people as up on up.id= r.knowreferedto 
	WHERE  emailstatus='0' AND  
	r.isdeleted <> '1' AND r.isdeleted <> '2'  AND knowenteredby  = '$user_id' and markrem='0' 
    and up.client_email <> u.user_email and user_status='1' and 
    (r.distance >=  0 and r.distance < '30') and distancecalculated='1' and r.ranking >='20' ");
	...
	if($allrecords->num_rows > 0)
	{
		//scanning distance
		$j = 0;
		while($temprow = $allrecords->fetch_array())
		{
			$trknowtorefer = $link->query(" SELECT  sum( ranking) as totalscore FROM user_rating where user_id in (" . $temprow['knowtorefer'] . "," 
			. $temprow['knowreferedto'] . " )  group by user_id"); 
			
			$rankrow = $trknowtorefer->fetch_array( );
			$sourcerank = $rankrow[0] ;
			$rankrow = $trknowtorefer->fetch_array( );
			$targetrank = $rankrow[0] ;
			 
			if( $sourcerank    >= 20 && $targetrank  >= 20   ): 
				 
				$refsuggestions[] =array('id' =>$temprow['id'] , 'partnerid' =>$temprow['partnerid'], 
				'knowtorefer'=>$temprow['knowtorefer'], 'knowreferedto'=>$temprow['knowreferedto'], 'ranking'=>$temprow['ranking'], 
				'entrydate'=>$temprow['entrydate'], 'emaillog'=>$temprow['emaillog'], 'emailstatus'=>$temprow['emailstatus'],
				'sourcezip' =>$temprow['sourcezip'], 'targetzip' =>$temprow['targetzip'], 'distance'=>$temprow['distance'], 
				'distancecalculated'=>$temprow['distancecalculated'], 'user_email' =>$temprow['user_email'], 'username'=>$temprow['username'], 
				'user_phone'=>$temprow['user_phone'],  'image'=>$temprow['image'], 'user_status' =>$temprow['user_status'], 'marked'=> '1' );
		...
		$pages = ceil(  $allrecordscount->fetch_array()['allcnt'] / 10);
		if($activepage > $pages)
		{
			$goto =  1; 
		}
		...
		$startfrom =  0 ;
		 ...
		 $html .='<table class="table table-condensed">
			<thead>
			<tr><th>Connect to suggest <a href="'.$help_data_buttons[4]['helpvideo'].'" target="_blank" ><i class="fa fa-arrow-right" ></i><span style="color:red;"> Help</span></a></th>
            <th>Partner Info <a href="'.$help_data_buttons[5]['helpvideo'].'" target="_blank" ><i class="fa fa-arrow-right" ></i><span style="color:red;"> Help</span></a></th>  
			<th>Introduced to <a href="'.$help_data_buttons[6]['helpvideo'].'" target="_blank" ><i class="fa fa-arrow-right" ></i><span style="color:red;"> Help</span></a></th>  
			<th>Action</th> 
			</tr>
		</thead><tbody>';
		
		$i=0; 
		//foreach( $refsuggestions  as $row)
		for($pos = $startfrom; $pos < sizeof($refsuggestions); $pos++)
		{ 
			$row = $refsuggestions[$pos];
			$str = "abcdefghijklmnopqrstuvwxyz";
    ...
    $html .= "<tr><td colspan='3'></td><td><input type='hidden' id='hidsf' data-sf='1' value='" . $row['id'] . "' ><input data-pagesize='10' data-pageno='". $goto ."' data-uid='" .  $uid  . "' type='button' class='btn btn-remove showreferrals' id='remintrosuggest' value='Delete Selected'/></td></td>";
	
	$lastpage = $pages ;
    $prev = $goto == 1 ? 1 : $goto-1;
    $next = $goto == $pages ? $pages : $goto+1; 
    $html .= "<tr><td colspan='5'><ul class='pagination pagiknows '><li><a data-key='$key' data-uid='$uid'  data-func='prev' data-pg='$prev'>«</a></li>";
    if( $goto > 10)
        $html .=  "<li><a data-key='$key' data-uid='$uid'   data-func='next' title='Show last few pages' data-pg='1'> ... </a></li>";
        ...
        if($goto < 10)
		{
			 for($j= 1 ; $j  <=  10  ; $j++)
			 {
				 ...
				 $html .= "<li class='$active'><a data-uid='$uid'  data-key='$key' data-state='$active'   data-pg='$j'>$j</a></li>";
			 }
		}
		else
		{
            for($i= $goto - 5; $i<= $goto + 4; $i++)
			{
				...
				$active = $i == $goto ? 'active' : '';
			    $html .= "<li class='$active'><a data-uid='$uid'  data-key='$key' data-state='$active'  data-pg='$i'>$i</a></li>";
			}
        }
...
if(isset($_POST['importknows']))
{
	...
	{ 
		...
		while($x <= $sheet['numRows'])  //cycle every row 
		{
			...
			$existing_know = $link->query("select count(*) as cnt from user_people where client_email='$email' and user_id='$user_id'");
			if($existing_know->num_rows  > 0)
			{
				$know_count = $existing_know->fetch_array()['cnt'] ;
				if($know_count == 0)
				{
...
if(isset($_POST['linkedinimport']))
{
	...
	$filename = $_SESSION['excelfileimport'];
    ...
    $apppath = ''; 
    $storeFolder =  'assets/uploads';    
    $targetPath = $_SERVER['DOCUMENT_ROOT'] . $ds. $apppath . $ds. $storeFolder . $ds . $filename ;
    ...
    else
    {
		...
		$arrayCount=$sheet['numRows'];
		
        while( $x < 100 && $x <= $sheet['numRows'])  //cycle every row for the first 100
        {
            $cname =  $sheet['cells'][$x][1] . " " . $sheet['cells'][$x][2] ;     
            ...
            {
				$existing_know = $link->query("select count(*) as cnt from mc_linkedin_import 
		...
		//update log 
		$status=0;			
		if( $x >= $arrayCount || $sheet['cells'][$x][1] == null )
		{
			$status =1;
			$arrayCount = $x;
		} 
		$link->query("update mc_upload_log set 
		total_imported= '$new' , 
		last_row_processed='$x', status='0', total_row= '$arrayCount' where filepath = '$filename'  "); 
...
if(isset($_POST['settings'] ))
{
	...
	if($_user_role == 'admin') {
        $vocation = $_POST['vocation'] ;
		$membervoc = $_POST['membervoc'] ;
        $settings = $_POST['settings'] ;
		if($settings ==1)
		{
			$reccount = $link->query(" select * from mc_common_vocation where  member_voc= '" .  $membervoc  . "'  " ); 
			if($reccount->num_rows  > 0)
			{ 
				$link->query("update mc_common_vocation set know_common_voc='$vocation'  where member_voc='" .  $membervoc  . "' ");
				$settinid = $link->affected_rows;
			}
			else 
			{
				$link->query("insert into mc_common_vocation ( member_voc , know_common_voc  ) values ('" .  $membervoc  . "', '" .  $vocation  . "' )");
...
if(isset($_POST['loadsettings'] ))
{
	...
	{ 
		...
		if($settings ==1)
		{
			$comvoc = $link->query(" select * from mc_common_vocation  order by member_voc   " );
			if($comvoc->num_rows  > 0)
			{
				 
				$html  =  "<h3>Existing Common Vocations</h3>
				<table class='table table-alternate'>
				<tr><th>Member Vocation</th><th>Common Vocations for Knows</th><th>Action</th></tr>";
				while($row = $comvoc->fetch_array()) 
				{
                    $html .= "<tr><td>" .  $row['member_voc']  . "</td><td>" .  $row['know_common_voc']  . "</td> ";
					$html .= "<td><button  data-src='" .  $row['member_voc']  . "' data-trg='" .  $row['know_common_voc']  . "' class='btn btn-xs btn-primary btneditcomvoc'>Edit</button></td></tr>";
                } 
			...
			{
				$html  =  '<p class="alertinfofix">No common vocations saved yet!</p>';
			}
		}      
    }  
	echo $html;  
...
if(isset($_POST['inviteknow']))
{  
    
    $id =$_POST['i']; 
	
	$result = $link->query(" select * from user_people where id='$id'  " );
 
    if($result->num_rows > 0)
    { 
		$stmt =  $pdo->prepare("insert into mc_claimprofile_invite (user_id, member_id, invitedate ) VALUES ( ?  ,'-1', NOW() )");
    ...
    if(  file_exists( $path . "templates/inviteknowtosignup.txt" ) )
    {
        $mailbody = file_get_contents( $path . "templates/inviteknowtosignup.txt" ) ;
        $mailbody = str_replace("{receipent}", $name , $mailbody ) ; 
         
		$mailbody  = str_replace("{mail_body}",  $mailbody  , $template_part ) ;
		
		sendmail(  $email , 'bob@mycity.com',  "Someone is looking at you on mycity.com. Respond by joing MyCity.Com now!",  $mailbody ,  $mailbody);
        echo $mailbody;
    }
    else 
    {
...
{
    $result = $link->query(" select id, username from mc_user where  id='$user_id'   " );
	if($result->num_rows > 0)
    {
		$row = $result->fetch_array();  
		$name =  $row['username'] ;
		...
		$ecount_rs = $link->query("select count(*) as cnt from mc_user where  id <> '$user_id'  and user_shortcode='$short_code_name'"); 
		$ecount_row = $ecount_rs->fetch_array();  
		$ecount = $ecount_row['cnt'];
		if( $ecount > 0)
		{
			$short_code_name = $short_code_name."-". ($ecount > 9 ? $ecount : "0".$ecount);
		} 
		$profileurl = 'https://www.mycity.com/profile/' . $short_code_name;
		$query = "update mc_user set  profileisvisible='1', publicprofile='$profileurl', user_shortcode='$short_code_name' WHERE id='$user_id' ";
		$link->query($query);  
	}
}
...
if(isset($_POST['minimalprofile']))
{
 ...
 if(isset($_POST['vcardon']))
{
	   
	$filename = $_SESSION['vcardfilename']; 
	$path = dirname(__DIR__)  . DIRECTORY_SEPARATOR . 'assets' .DIRECTORY_SEPARATOR . 'uploads' .DIRECTORY_SEPARATOR  . $filename ; 
	...
	$url = 'http://bcr1.intsig.net/BCRService/BCR_VCF2?PIN=&user=xanayaima@hotmail.com&pass=CQATR77PLCW3S9KK&json=1&lang=1';  
	$request =  curl_init();
...
if(isset($_POST['get_duplicate_referrals']))
{
	$user_id = $_POST['mid'];
    $page=$_POST['page'];
	getDuplicateReferences($user_id, $page ); 
}

if(isset($_POST['rem_duplicate_referrals']))
{
	$ids = $_POST['ids'];
    $data = json_decode(stripslashes( $ids  ));
	foreach($data as $d)
	{
		$link->query( "delete from user_people where id='" . $d . "'" ) ; 
	}
}
