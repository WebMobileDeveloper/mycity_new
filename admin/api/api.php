...
{ 		
	...
	$ds = DIRECTORY_SEPARATOR; 
	$apppath = '';
	$path =  $_SERVER['DOCUMENT_ROOT']  . $ds  . "edgeci" . $ds   ; 
	$html ='';
	...
	try
	{
		...
		{	
			...
			$jsonresult = array('error' =>  '0' ,   'mail' => $html,  'errmsg' =>  "Introduction email sent!"  ); 
		} 	
	}
	catch(PDOException $e)
	{
		$jsonresult = array('error' =>  '1' ,  'mail' => $html,   'errmsg' =>  $e->getMessage() ); 
	} 
	$response->getBody()->write(    json_encode($jsonresult)    );
	return $response;   
}); 
