...
if(!isset($_SESSION))session_start();
    ...
    include dirname(__FILE__).DIRECTORY_SEPARATOR . 'qrcode' .DIRECTORY_SEPARATOR   . "qrlib.php";    
...
$app->post('/member/know/updatephone/', function (Request $request, Response $response)
{
	...
	try
	{
		...
		$sql_query ='update user_people set client_phone=? where id=?'; 
		$stmt = $pdo->prepare($sql_query); 
...
$app->post('/member/searchnearest/', function (Request $request, Response $response)
{
	...
	{ 
		...
		if(sizeof($knows) > 0)
		{
			...
			for($i= $start ; $i <  $rst->rowCount() && $i < ($start + $pagesize) ; $i++)
			{
				$know_result[] = $knows[$i];  
			}
...
$app->post('/mails/sendintroducemail/', function (Request $request, Response $response)
{
	...
	$_username = $allPostPutVars['username']; 
	/*
	$suggestemail = "scade@cadecompany.com" ; 
	$suggestname = "Steve Cade";
    $suggestid =  "37516" ;
	$email =  "adamniec1@gmail.com";
	$profession = "Merchant Services" ; 
	$phone = "(440) 829-5868 (Mobi";
	$mailogid = "4934034";
	$receipentname = "Adam Niec"; 
	$receipentprof = "Business Manager,Business Owner,Business Owner- 1-10 Employees,Business Owner- 11-50 Employees,Business Owner- 51-200 Employees";
	$receipentphone = "760.634.5300";
	$clientid =  "59722"  ;	
    $cc1 = "paul@axxcesswealth.com" ;
    $ccname1 =  "Paul Hamzey" ; 
	$templateid = '13';
	*/
...
$app->post('/email/introduction/send/', function (Request $request, Response $response)
{
	...
	try
	{
		...
		{	
			...
			if($rst->rowCount() > 0)
			{
				$ratedby = $rst->fetchAll(PDO::FETCH_ASSOC)['username'];
				...
				{ 
					...
					{ 
						$pdo->query ("insert into referralsuggestions 
							( partnerid, knowtorefer,knowreferedto, entrydate,  knowenteredby, senton) 
								VALUES ('". $refcounter->fetchAll(PDO::FETCH_ASSOC)[0]['user_id'] . "', '$suggestid', 
								'$clientid' ,  NOW() ,  '$userid', NOW()  ) "); 
					} 
				}
				...
				//if( $ecnt )
				// if( $ecnt == 0)
				$stmt=$pdo->prepare("insert into mailbox (sender, receipent, subject, mailbody, senton, suggestedconnectid) VALUES 
				(?,?,?,?,NOW(), ? )");
				...
				<body>
					<p>Hello ' . $_username  . ',</p>
					<p>Your referral networking partner ' .  $suggestname  . 
					' is introduced to one of your connections that you gave a high rating. 
...
{  
	...
	$voclist = $allPostPutVars['vocations'];
	...
	try
	{
	   ...
	   if($rst->rowCount() > 0 )
	   {
		   ...
		   foreach($result as $row)
		   {
			   ...
			   if($rate_q->rowCount() > 0)
				   $result[$i]['ranking'] = $rate_q->fetchAll(PDO::FETCH_ASSOC)[0]['user_ranking'];  
				$i++;	   
		   }
...
{ 
	...
	try
	{
		...
		if($id == 0) 
		{
			...
			$pdo->query($sql_query);  
			$jsonresult = array('error' =>  '0' ,  'errmsg'  => 'Know information saved successfully!', 'knowid' => $knowid, 'action' => 'i' );   
		}
		else 
		{
			...
			client_lifestyle='$client_lifestyle', updatedate=NOW() ,tags =  '$client_tags', refgenerated='10' , showfirst='1'  WHERE id = '$id'");
			...
			$jsonresult = array('error' =>  '0' ,  'errmsg'  => 'Know information updated successfully!' , 'action' => 'u'    ); 
...
{ 
	...
	try
	{
		$jsonresult = array('error' =>  '1' ,  'errmsg' =>  sendreferralmail('xanayaima@gmail.com', 'Testing email', $body, $body) );  
