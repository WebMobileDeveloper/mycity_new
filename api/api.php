...
$app->post('/knows/search/',   function (Request $request, Response $response, $args){ 	$allPostPutVars = $request->getParsedBody();  
	...
	try
	{
	   ...
	   if($rst->rowCount() > 0 )
	   {
		   ...
		   $pages = ceil($result_count[0]['reccnt']/10);  
		   $i=0;
		   foreach($result as $row)		   {			   $rate_q  = $pdo->query("select SUM(ranking) as user_ranking from user_rating where user_id = '" . $row['knowid'] ."'");			   if($rate_q->rowCount() > 0)				   $result[$i]['ranking'] = $rate_q->fetchAll(PDO::FETCH_ASSOC)[0]['user_ranking'];  				   				if($row['client_zip'] == '' && $row['client_location'] !='' && sizeof($allcities) > 0)				{					for($ci=0; $ci < sizeof($allcities); $ci++)					{						if($allcities[$ci][0] == $row['client_location'])						{							$result[$i]['client_zip']  = $allcities[$ci][2] ; 							$pdo->query("update user_people set client_zip='" . $allcities[$ci][2] . "' where id = '" . $row['knowid'] ."'"); 							break;						} 					}				}				
				$i++;					
		   }
